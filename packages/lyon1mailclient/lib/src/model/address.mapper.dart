    ...
    return identical(this, other) ||
        (runtimeType == other.runtimeType &&
            AddressMapper.ensureInitialized()
                .isValueEqual(this as Address, other));
  }
...
class _AddressCopyWithImpl<$R, $Out>
  ...
  late final ClassMapperBase<Address> $mapper =
      AddressMapper.ensureInitialized();
  @override
  $R call({String? email, String? name}) => $apply(FieldCopyWithData(
      {if (email != null) #email: email, if (name != null) #name: name}));
  @override
  Address $make(CopyWithData data) => Address(
      data.get(#email, or: $value.email), data.get(#name, or: $value.name));
