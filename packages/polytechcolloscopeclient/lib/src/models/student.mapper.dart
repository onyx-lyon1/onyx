    ...
    return identical(this, other) ||
        (runtimeType == other.runtimeType &&
            StudentMapper.ensureInitialized()
                .isValueEqual(this as Student, other));
  }
...
class _StudentCopyWithImpl<$R, $Out>
  ...
  late final ClassMapperBase<Student> $mapper =
      StudentMapper.ensureInitialized();
  @override
  $R call({Year? year, String? name, int? id}) => $apply(FieldCopyWithData({
        if (year != null) #year: year,
        if (name != null) #name: name,
        if (id != null) #id: id
      }));
  @override
  Student $make(CopyWithData data) => Student(data.get(#year, or: $value.year),
      data.get(#name, or: $value.name), data.get(#id, or: $value.id));
